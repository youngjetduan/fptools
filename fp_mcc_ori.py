"""
This file (fp_mcc_ori.py) is designed for:
    MCC (Minutia Cylinder-Code) related functions (using orientation elements, which are different from basic MCC method)
...
class ORIMCC:
    def __init__(
        ...
        self.d_phi = np.arange(0.5, Nd) * self.delta_d - np.pi / 2  # [-pi/2, pi/2]
    ...
    def compute_similarity_matrix(self, des1, des2):
        ...
        delta = (des1["mnt"][:, 2][:, None] - des2["mnt"][:, 2][None] + 90) % 180 - 90
    ...
    def compute_binary_relation_fast(self, mnts):
        ...
        angle = normalize_minu_dir(temp - angle1, edge=90)
        t_angle = np.triu(angle, 1)
        angle = t_angle + t_angle.T

        r_angle = normalize_minu_dir(angle1.T - angle1, edge=90)
    ...
    def fingerprint_matching(self, mnts1, mnts2, des1, des2):
        ...
        angle_s = (
            1 - np.abs(normalize_minu_dir(angle1 - angle2, edge=90)) / self.thresholds["max_angle"][dist_range - 1]
        )
        angle_s = angle_s.clip(0, None)

        r_angle_s = (
            1 - np.abs(normalize_minu_dir(r_angle1 - r_angle2, edge=90)) / self.thresholds["max_rangle"][dist_range - 1]
        )
    ...
    def create_descriptor(self, mnts, img_shape, mask=None, is_save=False, fpath=None):
        ...
        for cur_ii, cur_mnt in enumerate(mnts):
            ...
            local_Ds = local_Ds.reshape(self.Ns, self.Ns, -1)
            local_Ds = np.minimum(local_Ds, local_Ds[::-1, ::-1]).reshape(self.Ns ** 2, -1)
            disk_mask = (disk_mask + disk_mask[::-1, ::-1]) > 0
            ...
            local_Dd = normalize_minu_dir((mnts[within_mask, 2] - cur_mnt[2]), edge=90) * np.pi / 180
            local_Dd = normalize_minu_dir(local_Dd[:, None] - self.d_phi[None], edge=np.pi / 2)
