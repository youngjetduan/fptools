"""
This file (manul_label_pose.py) is designed for:
...
class MainWidget(QWidget):
    ...
    def cur_pose(self, value):
        ...
        if np.any(self.__cur_pose != value):
            self.__edit_center_x.setText(f"{value[0]:.0f}")
    ...
    def fetch_new_data(self):
        ...
        if osp.exists(osp.join(self.__pose_prefix, f"{img_name}.txt")):
            self.cur_pose = np.loadtxt(osp.join(self.__pose_prefix, f"{img_name}.txt"))
        else:
